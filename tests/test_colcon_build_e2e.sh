...
pip install --break-system-packages -e "$REPO_ROOT" --quiet 2>&1 | tail -1
# Ensure colcon can build Python packages, discover them recursively,
# and generate setup.bash
pip install --break-system-packages colcon-python-setup-py colcon-bash colcon-recursive-crawl --quiet 2>&1 | tail -1
