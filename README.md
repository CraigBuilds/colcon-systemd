...
colcon-systemd generates from it.

### Input: `colcon-systemd.yaml`

Place this file in the source root of your package (next to `package.xml` or
`setup.py`), e.g. `~/my_ros2_ws/src/my_pkg/colcon-systemd.yaml`:

```yaml
services:
  - name: my_node
    entry_point: my_node
    description: "My ROS 2 talker node"
    environment:
      ROS_DOMAIN_ID: "42"
    ...
    args:
      - "--ros-args"
      - "-r"
      - "__node:=my_renamed_node"
```

### Output: Generated Wrapper Script

`~/my_ros2_ws/install/my_pkg/share/colcon-systemd/my_node.sh`
...
set -eo pipefail
source "/home/user/my_ros2_ws/install/setup.bash"
exec "/home/user/my_ros2_ws/install/my_pkg/lib/my_pkg/my_node" "$@"
```

### Output: Generated Service Unit

`~/my_ros2_ws/install/my_pkg/share/colcon-systemd/my_node.service`
...
[Service]
Type=simple
ExecStart=/home/user/my_ros2_ws/install/my_pkg/share/colcon-systemd/my_node.sh --ros-args -r __node:=my_renamed_node
Environment="ROS_DOMAIN_ID=42"
Environment="RCUTILS_LOGGING_SEVERITY=INFO"
Restart=on-failure
...
> **Note:** The paths above use `/home/user/my_ros2_ws` as a placeholder for
> your actual workspace root.  The real paths will reflect wherever you ran
> `colcon build`.
